    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-2 border-b border-white/[0.06]">
        ...
        <button
          onClick={() => executeTest()}
          disabled={loading}
          className="flex items-center gap-2 px-4 py-2 rounded-xl bg-rose-600 hover:bg-rose-500 text-white text-xs font-bold shadow-lg shadow-rose-500/20 transition-all cursor-pointer disabled:opacity-50"
        >
          ...
          return (
            <div
              ...
              className={`p-5 rounded-2xl border transition-all cursor-pointer ${
                isSelected
                  ? "border-sky-500 bg-[#25283b] shadow-lg shadow-sky-500/10"
                  : "border-white/[0.06] bg-[#1c1e2d] hover:bg-[#202235]"
              }`}
            >
              <div className="flex items-start justify-between">
                <div className="p-2 rounded-xl bg-[#151724] text-zinc-200 border border-white/[0.06]">
                  <Icon className="w-4 h-4 text-amber-400" />
                </div>
                <span className="text-[10px] font-semibold px-2 py-0.5 rounded-md bg-[#151724] text-zinc-400 border border-white/[0.06]">
                  {sc.tag}
                </span>
              </div>
              <h4 className="text-sm font-bold text-white mt-3.5">{sc.name}</h4>
      ...
      {result && (
        <div className="space-y-6">
          ...
          <div className={`rounded-2xl border p-5 ${
            cbStatus === "FROZEN" ? "bg-red-950/40 border-red-500/50 text-red-100" :
            cbStatus === "RED" ? "bg-rose-950/40 border-rose-500/50 text-rose-100" :
            "bg-amber-950/30 border-amber-500/40 text-amber-100"
          }`}>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex items-center gap-3.5">
                <div className="p-2.5 rounded-xl bg-black/40 border border-current/20">
              ...
              <div className="flex items-center gap-3">
                <Link
                  href="/audit-log"
                  className="px-4 py-2 rounded-xl bg-white/10 hover:bg-white/20 border border-white/20 text-xs font-semibold flex items-center gap-2 transition-colors"
                >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="p-4 rounded-2xl border border-white/[0.06] bg-[#1c1e2d]">
              ...
              <div className="text-xs text-zinc-400 font-mono mt-1">Base capital baseline</div>
            </div>
            ...
            <div className="p-4 rounded-2xl border border-white/[0.06] bg-[#1c1e2d]">
              ...
              <div className="text-xs text-zinc-400 font-mono mt-1">Net equity loss during shock</div>
            </div>
          ...
          <div className="rounded-2xl border border-white/[0.06] bg-[#1c1e2d] p-5 shadow-sm space-y-4">
